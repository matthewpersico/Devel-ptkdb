...
sub do_tabs {
    ...
    $w->add( 'Entry', -textvariable => \$tabs_str )->pack( -side => 'left' )
...
sub setup_search_panel {
    ...
    $frm->Button( -text => 'Goto', -command => sub { $self->DoGoto($entry) } )
      ->pack( -side => 'left' );
    $srchBtn = $frm->Button(
        -text    => 'Search',
...
sub configure_text {
    ...
    $txt->tagBind( "breakableLine", '<Button-1>',
        [ \&Devel::ptkdb::set_breakpoint_tag, $self, Ev('@'), 1 ] );
    $txt->tagBind( "breakableLine", '<Shift-Button-1>',
        [ \&Devel::ptkdb::set_breakpoint_tag, $self, Ev('@'), 0 ] );

    $txt->tagBind( "breaksetLine", '<Button-1>',
        [ \&Devel::ptkdb::clear_breakpoint_tag, $self, Ev('@') ] );
    $txt->tagBind( "breaksetLine", '<Shift-Button-1>',
        [ \&Devel::ptkdb::change_breakpoint_tag, $self, Ev('@'), 0 ] );

    $txt->tagBind( "breakdisabledLine", '<Button-1>',
        [ \&Devel::ptkdb::clear_breakpoint_tag, $self, Ev('@') ] );
    $txt->tagBind( "breakdisabledLine", '<Shift-Button-1>',
...
sub insertBreakpoint {
    ...
    while (@brks) {
        ...
        $self->{'text'}->tagAdd( $value ? "breaksetLine" : "breakdisabledLine",
...
sub insertTempBreakpoint {
    ...
    &DB::setdbline( $fname, $index + $offset,
...
sub removeBreakpointTags {
    ...
    foreach $brkPt (@brkPts) {
        ...
        $self->{'text'}
          ->tagAdd( "breakableLine", "$idx.0", "$idx.$Devel::ptkdb::linenumber_length" );
    }
  ...
  REF_CHECK: for ( ; ; ) {
    ...
    if ( $type eq 'ARRAY' or "$theRef" =~ /ARRAY/ ) {
        ...
        foreach $r ( @{$theRef} ) {
            ...
            push @$reusedRefs, $r;
            $result =
              $self->insertExpr( $reusedRefs, $dl, $r, "[$idx]", $depth - 1,
...
sub setupEvalWindow {
    ...
    $top->Button( -text => 'Dismiss', -command => $dismissSub )
      ->pack( -side => 'left', -fill => 'x', -expand => 1 );
    $top->Checkbutton( -text => 'Hex', -variable => \$self->{hexdump_evals} )
      ->pack( -side => 'left' );
...
sub RestoreState {
    ...
    $top = $DB::window->simplePromptBox( "Restore Config?",
